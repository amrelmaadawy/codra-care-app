...
class _WalkInPatientStepState extends State<WalkInPatientStep> {
  ...
  Widget _buildSearchTip(BuildContext context) {
    return Row(
      children: [
        Icon(Icons.info_outline_rounded, size: 14, color: context.textMutedColor),
        const SizedBox(width: 4),
        Text(
          'reception_booking.search_patient_tip'.tr(),
          style: AppTypography.bodySmall.copyWith(fontSize: 11, color: context.textMutedColor),
        ),
      ],
    );
  }
  ...
  Widget _buildNoPatientsFound(BuildContext context) {
    return Padding(
      ...
      child: Center(
        child: Column(
          children: [
            Icon(Icons.person_off_outlined, size: 36, color: context.textMutedColor),
            const SizedBox(height: AppSpacing.xs),
            Text(
